...
jest.mock('react-native-svg', () => {
  ...
  return {
    ...
    Text: ({ children, ...props }: any) =>
      React.createElement('Text', { ...props, testID: 'svg-text' }, children),
  };
});
...
describe('OutlinedText', () => {
  ...
  it('renders with optional height', () => {
    expect(() => {
      render(<OutlinedText text="Text without height" width={200} />);
    }).not.toThrow();
  });
});
...
describe('SvgTextOutlined', () => {
  ...
  it('handles text wrapping for long text', () => {
    const longText =
      'This is a very long text that should wrap to multiple lines when the width is limited';
    expect(() => {
  ...
  it('renders with different text anchors', () => {
    expect(() => {
      render(
        <SvgTextOutlined text="Anchored Text" width={200} textAnchor="end" />
      );
    }).not.toThrow();
  });
});
